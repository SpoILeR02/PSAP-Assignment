...
void showGotchaMessage(int runnersPosition) {
	cout << "| Runner #1 and Runner #2 Current Location: " << setw(24) << left << setfill(' ') << runnersPosition << '|' << endl;
	cout << '|' << setw(67) << left << setfill(' ') << " Track:" << '|' << endl;
	cout << "| " << setw(60) << right << setfill('-') << '|' << setw(5) << '-' << " |" << endl;
	if (runnersPosition < 60)
	{
		cout << "| " << setw(runnersPosition) << right << setfill(' ') << 1 << setw(60 - runnersPosition) << '|' << setw(7) << " |" << endl;
		cout << "| " << setw(runnersPosition) << right << setfill(' ') << 2 << setw(60 - runnersPosition) << '|' << setw(7) << " |" << endl;
	}
	else if (runnersPosition == 60)
	{
		cout << "| " << setw(60) << right << setfill(' ') << 1 << setw(5) << ' ' << " |" << endl;
		cout << "| " << setw(60) << right << setfill(' ') << 2 << setw(5) << ' ' << " |" << endl;
	}
	else
	{
		cout << "| " << setw(60) << right << setfill(' ') << '|' << setw(abs(60 - runnersPosition)) << 1 << setw(67 - runnersPosition) << " |" << endl;
		cout << "| " << setw(60) << right << setfill(' ') << '|' << setw(abs(60 - runnersPosition)) << 2 << setw(67 - runnersPosition) << " |" << endl;
	}
	cout << "| " << setw(60) << right << setfill('-') << '|' << setw(5) << '-' << " |" << endl;
	cout << '|' << setw(67) << left << setfill('=') << '=' << '|' << endl;
...
int main() {
	...
	while (true)
	{
		...
		position_runnerOne = calculateRunnerPosition(position_runnerOne, actionNumberRunnerOne);
		int randomNumberTwo = randomIntegerGenerator();
		int actionNumberRunnerTwo = verifyRunnersAction(false, randomNumberTwo);
		position_runnerTwo = calculateRunnerPosition(position_runnerTwo, actionNumberRunnerTwo);
		...
		if (position_runnerOne == position_runnerTwo && position_runnerOne != 1)
		{
			...
			showActionMessage(position_runnerOne, actionNumberRunnerOne);
			cout << '|' << setw(67) << left << setfill('=') << '=' << '|' << endl;
			cout << "| Runner 2 Rolled Number: " << setw(42) << left << setfill(' ') << randomNumberTwo << '|' << endl;
			showActionMessage(position_runnerTwo, actionNumberRunnerTwo);
			cout << '|' << setw(67) << left << setfill('=') << '=' << '|' << endl;
			showGotchaMessage(position_runnerOne);
		}
		else
		{
			cout << "| Runner 1 Rolled Number: " << setw(42) << left << setfill(' ') << randomNumberOne << '|' << endl;
